...
async def upload(request, pdf_file: UploadFile):
    ...
    try:
        ...
        if not success: return Toast(f"Could not extract highlights: {error}", alert_cls=AlertT.error, cls=(ToastVT.top, ToastHT.end))
        ...
        return Div(
            Toast(
                f"✅ Extracted {len(new_highlights)} highlights from {pdf_file.filename}",
...
def do_login(email: str, pwd: str, session):
    ...
    if attempts_left > 0:
        return Toast(
            ...
            alsert_cls=AlertT.error, cls=(ToastVT.top, ToastHT.end)
        )
