...
# %% auto 0
__all__ = ['config', 'db', 'highlights_db', 'users_db', 'ph', 'beforeware', 'app', 'rt', 'AppConfig', 'HighlightData',
           ...
           'User', 'hash_pwd', 'verify_pwd', 'get_current_user', 'validate_email', 'check_rate_limit',
           'record_failed_attempt', 'clear_rate_limit', 'validate_web_highlight', 'auth_before', 'index', 'view_source',
           'upload', 'save_web_highlight', 'login_page', 'redirect_home', 'do_login', 'signup_page', 'do_signup',
...
def extract_highlights(pdf: Union[str, Path, bytes]):
    ...
    try:
        ...
        if "damaged" in error_msg.lower() or "corrupt" in error_msg.lower(): False, "PDF file appears to be corrupted or damaged."
...
async def upload(request, pdf_file: UploadFile):
    ...
    try:
        ...
        if len(content) > config.max_pdf_size_bytes: return Alert(f"PDF too large. Maximum size is {config.max_pdf_size_mb}MB", cls=AlertT.error)
        
        success, result = extract_highlights(content)

        if not success: return Alert(f"Could not extract highlights: {error}", cls=AlertT.error)
        if not result: return Alert("No highlights found in this PDF", cls=AlertT.warning)
...
async def save_web_highlight(text:str, url:str, color:str='#ffff00', 
    ...
    if not success: return {'status': 'error', 'message': error}
...
def do_signup(email: str, password: str, confirm_password: str, session):
    """Handle signup"""
    print("🔵 do_signup() called") 
    
    if not validate_email(email): return Alert("Please enter a valid email address", cls=AlertT.error)
    ...
    if not success: 
        ...
        return Alert(result, cls=AlertT.error)
